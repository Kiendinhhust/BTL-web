      ...
      try {
        ...
        const productsWithImages = await Promise.all(productsData.map(async (product) => {
          try {
            ...
            let items = [];
            console.log(`Items response for product ${product.product_id}:`, itemsResponse.data);

            if (itemsResponse.data && itemsResponse.data.data && Array.isArray(itemsResponse.data.data.items)) {
              items = itemsResponse.data.data.items;
              console.log(`Using data.data.items for product ${product.product_id}`);
            } else if (itemsResponse.data && itemsResponse.data.data && itemsResponse.data.data.items) {
              // Handle case where items might be an object instead of array
              console.log(`Items is not an array, converting for product ${product.product_id}`);
              items = [itemsResponse.data.data.items];
            } else if (Array.isArray(itemsResponse.data)) {
              items = itemsResponse.data;
              console.log(`Using direct array for product ${product.product_id}`);
            } else if (itemsResponse.data && typeof itemsResponse.data === 'object') {
              ...
              if (itemsResponse.data.success && Array.isArray(itemsResponse.data.data)) {
                ...
                console.log(`Using data array for product ${product.product_id}`);
              }
            } else {
              console.log(`Unexpected items response format for product ${product.product_id}:`, itemsResponse.data);
            }

            console.log(`Final items for product ${product.product_id}:`, items);
            ...
            if (Array.isArray(items) && items.length > 0) {
              try {
                ...
                console.error(`Error processing items for product ${product.product_id}:`, mapError);
                processedItems = [];
              }
            } else {
              console.log(`No valid items to process for product ${product.product_id}`);
            }
            ...
            console.error(`Error fetching items for product ${product.product_id}:`, error);
            return product;
          }
        }));
