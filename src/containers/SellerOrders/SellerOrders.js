...
import { getShopOrders, getOrderDetails, updateOrderStatus } from '../../services/orderService';
...
const SellerOrders = (props) => {
  ...
  const handleUpdateStatus = (orderId, newStatus) => {
    if (window.confirm(`Bạn có chắc chắn muốn cập nhật trạng thái đơn hàng thành "${getStatusInfo(newStatus).text}"?`)) {
              ...
              <div className="table-body">
                {orders.map(order => (
                  <div key={order.order_id} className="table-row">
                    ...
                    <div className="cell customer">{order.user?.username || 'Không xác định'}</div>
                    ...
                    <div className="cell total">{formatPrice(order.total_price)} VNĐ</div>
              ...
              <div className="modal-body">
                <div className="order-status-section">
                  ...
                  <div className="current-status">
                    ...
                    <span className={`status-badge ${getStatusInfo(selectedOrder.status).class}`}>
                      ...
                      {selectedOrder.status === 'pending' && (
                        <>
                          ...
                          <button
                            ...
                            onClick={() => handleUpdateStatus(selectedOrder.order_id, 'canceled')}
                          >
                            Hủy đơn hàng
                          </button>
                        </>
                  ...
                  <div className="order-info">
                    ...
                    <div className="info-grid">
                      <div className="info-item">
                        ...
                        <span className="info-value">{selectedOrder.order_code}</span>
                      </div>
                      <div className="info-item">
                      ...
                      <div className="info-item">
                        ...
                        <span className="info-value">{selectedOrder.user?.username || 'Không xác định'}</span>
                      </div>
                      <div className="info-item">
                        <span className="info-label">Tổng tiền:</span>
                        <span className="info-value">{formatPrice(selectedOrder.total_price)} VNĐ</span>
                      </div>
                    </div>
                  </div>
                          ...
                          {selectedOrder.orderShipping.shippingAddress ? (
                            <div className="address-info">
                              <p className="address-type">
                                {selectedOrder.orderShipping.shippingAddress.type === 'home' ? 'Nhà riêng' :
                                 selectedOrder.orderShipping.shippingAddress.type === 'office' ? 'Văn phòng' : 'Địa chỉ khác'}
                              </p>
                              <p className="address-line">{selectedOrder.orderShipping.shippingAddress.address_infor}</p>
                            </div>
                          ) : (
                        ...
                        <div className="shipping-method">
                          ...
                          {selectedOrder.orderShipping.shippingMethod ? (
                            <div className="method-info">
                              <p className="method-name">{selectedOrder.orderShipping.shippingMethod.name}</p>
                              <p className="method-delivery">
                                Thời gian giao hàng: {selectedOrder.orderShipping.shippingMethod.min_delivery_days} - {selectedOrder.orderShipping.shippingMethod.max_delivery_days} ngày
                              </p>
                              <p className="method-cost">{formatPrice(selectedOrder.orderShipping.shipping_cost)} VNĐ</p>
                            </div>
                          ) : (
