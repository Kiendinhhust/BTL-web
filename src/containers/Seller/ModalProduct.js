...
class ModalProduct extends Component {
    ...
    async componentDidMount() {
        if (this.props.productData) {
            const { title, description, category_id, status } = this.props.productData;
    ...
    handleItemImageUpload = async (event, index) => {
        ...
        if (file.size > 5 * 1024 * 1024) {
            ...
            errors.items[index].image = 'Kích thước file quá lớn. Vui lòng chọn file nhỏ hơn 5MB.';
    ...
    validateForm = () => {
        ...
        this.state.items.forEach((item, index) => {
            ...
            if (item.sale_price && (isNaN(item.sale_price) || parseFloat(item.sale_price) <= 0)) {
    ...
    handleSubmit = async (e) => {
        ...
        try {
          ...
          if (this.props.productData?.product_id) {
            res = await updateProduct(this.props.productData.product_id, productData);
          } else {
          ...
          toast.success(this.props.productData ? 'Cập nhật thành công!' : 'Thêm sản phẩm thành công!');
    ...
    render() {
        ...
        const { title, description, status, items, currentItemIndex, errors, isSubmitting } = this.state;
        ...
        const itemErrors = errors.items && errors.items[currentItemIndex] ? errors.items[currentItemIndex] : {};

        return (
            <Modal isOpen={isOpen} toggle={toggleModal} className="modal-product-container" size="lg">
                <div className={isSubmitting ? 'loading-container' : ''}>
                                ...
                                <div className="item-tabs">
                                    <div className="item-tabs-header">
                                        {items.map((itemData, index) => (
                                            <div
                                                key={index}
                                                className={`item-tab ${index === currentItemIndex ? 'active' : ''}`}
                                                onClick={() => this.setState({ currentItemIndex: index })}
                                            >
                                        ...
                                        <div className="form-row">
                                            <div className="form-group col-md-12">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(event) => this.handleItemChange(currentItemIndex, 'sku', event.target.value)}
                                                    className={itemErrors.sku ? 'form-control is-invalid' : 'form-control'}
                                                    placeholder="Nhập mã SKU (VD: RED-M, BLUE-L)"
                                                />
                                                {itemErrors.sku && <div className="invalid-feedback">{itemErrors.sku}</div>}
                                            </div>
                                        </div>
                                        ...
                                        <div className="form-row">
                                            <div className="form-group col-md-4">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(event) => this.handleItemChange(currentItemIndex, 'price', event.target.value)}
                                                    className={itemErrors.price ? 'form-control is-invalid' : 'form-control'}
                                                />
                                                {itemErrors.price && <div className="invalid-feedback">{itemErrors.price}</div>}
                                            </div>
                                            ...
                                            <div className="form-group col-md-4">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(event) => this.handleItemChange(currentItemIndex, 'sale_price', event.target.value)}
                                                    className={itemErrors.sale_price ? 'form-control is-invalid' : 'form-control'}
                                                />
                                                {itemErrors.sale_price && <div className="invalid-feedback">{itemErrors.sale_price}</div>}
                                            </div>
                                            ...
                                            <div className="form-group col-md-4">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(event) => this.handleItemChange(currentItemIndex, 'stock', event.target.value)}
                                                    className={itemErrors.stock ? 'form-control is-invalid' : 'form-control'}
                                                />
                                                {itemErrors.stock && <div className="invalid-feedback">{itemErrors.stock}</div>}
                                            </div>
                                        </div>
                                            ...
                                            <div className="attributes-container">
                                                <div className="attribute-row">
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            const oldKey = Object.keys(currentItem.attributes)[0] || '';
                                                            const oldValue = currentItem.attributes[oldKey] || '';
                                                            const newAttributes = {};
                                                            if (e.target.value) {
                                                                newAttributes[e.target.value] = oldValue;
                                                            }
                                                            this.handleItemChange(currentItemIndex, 'attributes', newAttributes);
                                                        }}
                                                    />
                                                    <input
                                                        type="text"
                                                        ...
                                                        value={Object.values(currentItem.attributes)[0] || ''}
                                                        onChange={(e) => {
                                                            const key = Object.keys(currentItem.attributes)[0] || '';
                                                            if (key) {
                                        ...
                                        <div className="form-group">
                                            ...
                                            <div className="upload-image">
                                                <input
                                                    ...
                                                    type="file"
                                                    hidden
                                                    onChange={(event) => this.handleItemImageUpload(event, currentItemIndex)}
                                                />
                                                <label className="upload-button" htmlFor={`itemImage-${currentItemIndex}`}>
                                                ...
                                                <div className="preview-image">
                                                    {currentItem.previewImgURL ? (
                                                        <div
                                                            className="preview"
                                                            style={{ backgroundImage: `url(${currentItem.previewImgURL})` }}
                                                        ></div>
                                                    ) : (
                                                ...
                                                {itemErrors.image && <div className="text-danger mt-2">{itemErrors.image}</div>}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <ModalFooter>
                        ...
                        <Button color="primary" onClick={this.handleSubmit} disabled={isSubmitting}>
