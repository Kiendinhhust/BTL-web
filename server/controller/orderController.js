...
const createOrder = async (req, res) => {
  ...
  // Xác thực thông tin
   ...
   for (const item of orderItemsInput) if (!item.item_id || !item.quantity || item.quantity <= 0) return res.status(400).json({ message: "Thông tin mặt hàng không hợp lệ." });
  ...
  try {
          ...
          try {
               ...
               if (db.Cart) {
                   cartClearedCount = await Cart.destroy({
                    ...
                    console.log(`Cleared ${cartClearedCount} items from cart for user ${userId}`);
               } else {
      ...
      // --- 12. Phản hồi ---
       const createdOrderDetails = await Order.findByPk(newOrder.order_id, { include: [/*...*/]}); // Keep includes
      res.status(201).json({...createdOrderDetails.get({ plain: true }), cartClearedCount}); // Thêm thông tin đã xóa cart
