...
const createProduct = async (req, res) => {
  try {
    ...
    res.status(500).send({ message: error.message || "Đã xảy ra lỗi khi tạo sản phẩm." });
  }
};
...
const getAllProducts = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({ message: error.message || "Đã xảy ra lỗi khi lấy danh sách sản phẩm." });
  }
};
...
const updateProduct = async (req, res) => {
  ...
  try {
    ...
    if (num == 1) { // Sequelize update trả về mảng với số dòng bị ảnh hưởng
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
        return res.status(400).send({ message: `Sản phẩm với tiêu đề mới đã tồn tại (slug bị trùng).` });
    }
    res.status(500).send({ message: `Lỗi khi cập nhật sản phẩm với id=${id}.` });
  }
};
...
const checkProductOwnership = async (productId, userId) => {
    ...
    if (!product) return { product: null, isOwner: false, error: 'Sản phẩm không tồn tại.' };
...
const createItem = async (req, res) => {
    ...
    if (sale_price && parseFloat(sale_price) >= parseFloat(price)) {
        return res.status(400).send({ message: "Giá khuyến mãi phải nhỏ hơn giá gốc." });
    }
    // Thuộc tính
    if (attributes && typeof attributes !== 'object') {
         return res.status(400).send({ message: "Thuộc tính (attributes) phải là một đối tượng JSON." });
    }
...
const getItemsByProduct = async (req, res) => {
    ...
    try {
        ...
        res.status(500).send({ message: "Đã xảy ra lỗi khi lấy danh sách mặt hàng." });
    }
};
