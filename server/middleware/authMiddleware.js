...
const authenticateToken = async (req, res, next) => {
  ...
  if (token == null) {
    ...
    return res
      .status(401)
      .json({ error: "Yêu cầu chưa được xác thực (Không tìm thấy token)." });
  }
  ...
  try {
    ...
    if (!user) {
      ...
      return res
        .status(403)
        .json({ error: "Token hợp lệ nhưng không tìm thấy người dùng." });
    }
    ...
    if (error.name === "TokenExpiredError") {
      return res
        .status(403)
        .json({ error: "Token đã hết hạn.", refress: true });
    }
...
const authorizeRole = (allowedRoles) => {
  return (req, res, next) => {
    ...
    if (!req.user || !req.user.role) {
      ...
      return res
        .status(403)
        .json({ error: "Không thể xác định vai trò người dùng." });
    }
